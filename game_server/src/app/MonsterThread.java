...
public class MonsterThread implements ChannelHandler {
	...
	public void ActiveMonster(String targetName) {
		if (!IsActive) {
			try {
				...
				App.server.channel.pipeline().firstContext().writeAndFlush(
						new DatagramPacket(Unpooled.copiedBuffer(MosterStateString, CharsetUtil.UTF_8), MyAddress));
	...
	public void MonsterLogic() {
		...
		try {
			...
			Vector3d CurrentDirectionVector = new Vector3d(FirstPlayerVector.getX() - MyVector.getX(),
					FirstPlayerVector.getY() - MyVector.getY(), 0);
			CurrentDirectionVector.normalize();

			Vector3d TargetToDirectionVector = new Vector3d(TargetVector.getX() - MyVector.getX(),
					TargetVector.getY() - MyVector.getY(), 0);
